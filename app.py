...
with l:
    ...
    # 💡 修正 1：移除 key 綁定。直接將 session_state 作為初始 value。
    # 這樣編輯器會獨立處理輸入，計算則抓取 ed_df。
...
with r:
    ...
    with st.container(border=True):
        ...
        c1, c2 = st.columns(2)
        with c1: dt = st.text_input("Date", value="2025/12/30" if st.session_state.show_img else "")
        with c2: ut = st.text_input("Unit", value="mm" if st.session_state.show_img else "")
    
    st.markdown('<p class="section-label">⌨️ Target Spec (±)</p>', unsafe_allow_html=True)
    with st.container(border=True):
        # 💡 修正 2：直接使用變數 ts 接收輸入，移除重複賦值行
        ts = st.number_input("Target Spec", 
    ...
    with st.container(border=True):
        ...
        st.markdown('<div class="made-by-leo-Oliver" style="text-align:right; font-size:10px; color:#aaa;">App Made by Leo & Oliver</div>', unsafe_allow_html=True)
